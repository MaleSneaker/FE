...
const UpdateProduct = () => {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      const res = await getDetailProduct(id as string);
      const category = res?.category as unknown as { _id: string };
      const brand = res?.brand as unknown as { _id: string };
      const imagesFile = res.images.map((item, index) => {
        return {
          uid: String(index + item),
          status: "done",
          url: item,
        };
      });
  ...
  const onFinish = async (values: any) => {
    ...
    try {
      ...
      const newFiles = values.images.filter((item: any) => item.originFileObj);
      ...
      const images: string[] = [...oldImages, ...newUrls];
      const [thumbnail] = await Promise.all(
        values.thumbnail.map(async (item: any) => {
          if (item.url) {
            return item.url;
          }
      ...
      const res = await updateProduct(id as string, payload);
      if (res) {
        console.log(res);
